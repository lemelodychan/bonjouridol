...
export async function GET(request) {
  ...
  const destination = redirectMap[cleanPath]
  
  if (destination) {
    console.log(`Redirecting ${cleanPath} to ${destination}`)
    return NextResponse.redirect(new URL(destination, request.url), 301)
  }
  
  // If no redirect found, return 404
  return NextResponse.json({ error: 'No redirect found for this path' }, { status: 404 })
}
