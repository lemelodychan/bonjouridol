...
export async function POST(request) {
  ...
  // Fetch raw items oldest-first so we always clear the backlog in order
  ...
  // Artist names for prompt context (fail-safe: continue without them)
  ...
  // Recent feedback for few-shot injection
  ...
  for (const item of items) {
    try {
      ...
      // Leave item as 'raw' so the next process run picks it up
