...
async function createSupabaseEntryFromPrismic(prismicDoc, supabase) {
  try {
    ...
    if (prismicDoc.documentId) {
      ...
      if (existingByDocId) {
        ...
        return {
          ...
          galleryData: existingByDocId.gallery_data,
    ...
    const releaseTitle = prismicDoc.releaseTitle || `New Galleries - ${new Date(prismicDoc.createdAt || Date.now()).toISOString().split('T')[0]} - ${prismicDoc.title || 'Untitled'} - ${timestamp}`
    
    const { data, error } = await supabase
      .from('pending_gallery_migrations')
      .insert({
        uid: prismicDoc.uid,
        title: prismicDoc.title,
        release_title: releaseTitle,
        document_id: prismicDoc.documentId,
        repository_name: prismicDoc.repositoryName || 'bonjouridol',
        gallery_data: prismicDoc.galleryData || null,
        status: 'pending',
...
async function createPrismicDocumentFromSupabase(supabaseEntry) {
  try {
    ...
    const galleryData = supabaseEntry.galleryData || {}
    ...
    const releaseTitle = supabaseEntry.releaseTitle || `New Galleries - ${releaseDate} - ${supabaseEntry.title || 'Untitled'} - ${timestamp}`
    ...
    const documentToSend = {
      ...
      lang: 'en-us',
      title: supabaseEntry.title || 'Untitled',
      data: data,
      release_title: releaseTitle,
